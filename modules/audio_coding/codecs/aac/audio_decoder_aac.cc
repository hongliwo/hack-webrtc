...
AudioDecoderAACImpl::AudioDecoderAACImpl() {
  WebRtcAAC_CreateDecoder(&dec_state_);
  WebRtcAAC_DecoderInit(dec_state_);
}

AudioDecoderAACImpl::~AudioDecoderAACImpl() {
  WebRtcAAC_FreeDecoder(dec_state_);
}

bool AudioDecoderAACImpl::HasDecodePlc() const {
  return false;
}
...
int AudioDecoderAACImpl::DecodeInternal(const uint8_t* encoded,
	...
	RTC_LOG(LS_INFO) << "### AAC DecodeInternal, encoded_len:"<< encoded_len<<",sample_rate_hz:"<<sample_rate_hz<<",speech_type:"<<speech_type;
  RTC_DCHECK_EQ(SampleRateHz(), sample_rate_hz);
  int16_t temp_type = 1;  // Default is speech.
  size_t ret =
      WebRtcAAC_Decode(dec_state_, encoded, encoded_len, decoded, &temp_type);
  *speech_type = ConvertSpeechType(temp_type);
  RTC_LOG(LS_INFO) << "### AAC DecodeInternal stop" <<",ret:"<<ret<<",speech_type:"<<*speech_type;
...
std::vector<AudioDecoder::ParseResult> AudioDecoderAACImpl::ParsePayload(
    rtc::Buffer&& payload,
    uint32_t timestamp) {
	RTC_LOG(LS_INFO) << "### AudioDecoderAACImpl::ParsePayload payload.size():"<<payload.size()<<",timestamp:"<<timestamp;
...
AudioDecoderAACStereoImpl::AudioDecoderAACStereoImpl() {
  WebRtcAAC_CreateDecoder(&dec_state_left_);
  WebRtcAAC_CreateDecoder(&dec_state_right_);
  WebRtcAAC_DecoderInit(dec_state_left_);
  WebRtcAAC_DecoderInit(dec_state_right_);
}

AudioDecoderAACStereoImpl::~AudioDecoderAACStereoImpl() {
  WebRtcAAC_FreeDecoder(dec_state_left_);
  WebRtcAAC_FreeDecoder(dec_state_right_);
}

int AudioDecoderAACStereoImpl::DecodeInternal(const uint8_t* encoded,
                                               size_t encoded_len,
                                               int sample_rate_hz,
                                               int16_t* decoded,
                                               SpeechType* speech_type) {
	RTC_LOG(LS_INFO) << "### DecodeInternal Stereo";
  RTC_DCHECK_EQ(SampleRateHz(), sample_rate_hz);
  ...
  *speech_type = ConvertSpeechType(temp_type);
  ...
  RTC_LOG(LS_INFO) << "### DecodeInternal Stereo stop";
  return static_cast<int>(ret);
}

int AudioDecoderAACStereoImpl::PacketDuration(const uint8_t* encoded,
                                               size_t encoded_len) const {
...
int AudioDecoderAACStereoImpl::SampleRateHz() const {
  return 16000;
}

size_t AudioDecoderAACStereoImpl::Channels() const {
  return 2;
}

void AudioDecoderAACStereoImpl::Reset() {
  WebRtcAAC_DecoderInit(dec_state_left_);
  WebRtcAAC_DecoderInit(dec_state_right_);
}

std::vector<AudioDecoder::ParseResult> AudioDecoderAACStereoImpl::ParsePayload(
    rtc::Buffer&& payload,
    uint32_t timestamp) {
	RTC_LOG(LS_INFO) << "### AudioDecoderAACStereoImpl::ParsePayload payload.size():"<<payload.size();
  return LegacyEncodedAudioFrame::SplitBySamples(this, std::move(payload),
                                                 timestamp, 2 * 8, 16);
}
...
// in the output array.
void AudioDecoderAACStereoImpl::SplitStereoPacket(
